...
1. Создание и управление товарами/услугами:
    1. продавцы могут добавлять новые товары и услуги, указывая такие характеристики, как название, описание, цена и загрузка изображений. Эти операции осуществляются через `POST /productlist`.
2. Редактирование данных о товарах/услугах:
    1. через API-продавцы могут вносить изменения в существующие записи товаров/услуг (редактировать название, описание, цену, заменять изображения). Это реализуется методом `PUT` или `PATCH` запросов к соответствующим ресурсам `PUT /productlist/{id}`, `PATCH /productlist/{id}`.
...
2. Просмотр ассортимента и карточек товаров:
    1. покупатели могут просматривать полный перечень товаров и услуг, а также детальную информацию о конкретном товаре через соответствующие запросы `GET /productlist`, `GET /productlist/{product_id}`
